...
export default function DashboardPage() {
    ...
    const expandAll = () => {
        ...
        expenseRows.forEach((_, i) => { newState[`exp-${i}`] = true; });
    ...
    const { incomeRows, expenseRows, monthlyIncomeTotals, monthlyExpenseTotals } = useMemo(() => {
        ...
        safeTransactions.forEach(t => {
            ...
            if (t.type === 'INCOME') {
                ...
                const catName = t.category?.name || (typeof t.category === 'string' ? t.category : "Otros Ingresos");
        ...
        const iRows = Object.entries(incomeMap).map(([name, values]) => ({ name, values }));
    ...
    const maxMonthlyValue = Math.max(...monthlyIncomeTotals, ...monthlyExpenseTotals, 1);
        ...
        return (
            <Appshell>
                <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
                    ...
                    <p className="text-gray-400 font-black uppercase tracking-widest animate-pulse">Sincronizando Resumen...</p>
                </div>
            </Appshell>
        );
    }
        ...
        <Appshell>
            <div className="flex flex-col gap-10 animate-fade-in-up pb-20">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1 font-semibold flex items-center gap-2">
                                ...
                                Análisis estructural de ingresos y gastos {new Date().getFullYear()}
                            </p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-emerald-50 flex flex-col relative overflow-hidden group hover:-translate-y-1 transition-all duration-500">
                        ...
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                <p className="text-xs font-black text-emerald-600/60 uppercase tracking-[0.2em] mb-2">Ingresos Anuales</p>
                                <h3 className="text-4xl font-black text-gray-900">
                                    <span className="text-emerald-500 text-2xl mr-1">S/</span>
                                    {showValues ? grandTotalIncome.toLocaleString('en-US', { minimumFractionDigits: 2 }) : "••••••"}
                                </h3>
                                <div className="mt-4 flex items-center gap-2 text-emerald-600 font-bold text-sm bg-emerald-50 px-3 py-1 rounded-full w-fit">
                                    <TrendingUp className="w-4 h-4" /> +{((grandTotalIncome / (grandTotalExpense || 1)) * 100).toFixed(0)}% vs Gastos
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-rose-50 flex flex-col relative overflow-hidden group hover:-translate-y-1 transition-all duration-500">
                        ...
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                <p className="text-xs font-black text-rose-600/60 uppercase tracking-[0.2em] mb-2">Gastos Totales</p>
                                <h3 className="text-4xl font-black text-gray-900">
                                    <span className="text-rose-500 text-2xl mr-1">S/</span>
                                    {showValues ? grandTotalExpense.toLocaleString('en-US', { minimumFractionDigits: 2 }) : "••••••"}
                                </h3>
                                <div className="mt-4 flex items-center gap-2 text-rose-600 font-bold text-sm bg-rose-50 px-3 py-1 rounded-full w-fit">
                                    <TrendingDown className="w-4 h-4" /> {((grandTotalExpense / (grandTotalIncome || 1)) * 100).toFixed(0)}% del Ingreso
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-indigo-50 flex flex-col relative overflow-hidden group hover:-translate-y-1 transition-all duration-500">
                        ...
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                <p className="text-xs font-black text-indigo-600/60 uppercase tracking-[0.2em] mb-2">Ahorro Neto</p>
                                <h3 className="text-4xl font-black text-gray-900">
                                    <span className="text-indigo-500 text-2xl mr-1">S/</span>
                                    {showValues ? grandTotalBalance.toLocaleString('en-US', { minimumFractionDigits: 2 }) : "••••••"}
                                </h3>
                                <div className={`mt-4 flex items-center gap-2 font-bold text-sm px-3 py-1 rounded-full w-fit ${grandTotalBalance >= 0 ? 'bg-indigo-50 text-indigo-600' : 'bg-red-50 text-red-600'}`}>
                                    <Wallet className="w-4 h-4" /> {grandTotalBalance >= 0 ? 'Saldo Positivo' : 'Déficit Acumulado'}
                                </div>
                            </div>
                ...
                <div className="bg-white rounded-[2rem] md:rounded-[3.5rem] p-6 md:p-12 shadow-[0_30px_70px_rgba(0,0,0,0.03)] border border-gray-50 flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-16 gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-gray-800 tracking-tight">Comparativa de Rendimiento Mensual</h2>
                                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Análisis de flujo mensual acumulado</p>
                            </div>
                        </div>
                        <div className="flex bg-gray-50 p-2 rounded-2xl border border-gray-100 shadow-inner">
                            <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-xl shadow-sm border border-gray-100">
                                ...
                                <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Ingresos</span>
                            </div>
                            <div className="flex items-center gap-3 px-4 py-2">
                                <div className="w-3 h-3 bg-rose-500 rounded-full"></div>
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Egresos</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="overflow-x-auto custom-scrollbar-premium pb-4 -mx-2 px-2 md:mx-0 md:px-0">
                        <div className="relative h-80 min-w-[700px] md:min-w-full w-full group">
                            ...
                            {months.map((m, i) => {
                                const incHeight = (monthlyIncomeTotals[i] / maxMonthlyValue) * 100;
                                const expHeight = (monthlyExpenseTotals[i] / maxMonthlyValue) * 100;
                                ...
                                return (
                                    <div key={`chart-v2-${m}`} className="flex flex-col items-center flex-1 h-full group/bar relative">
                                        <div className="flex items-end justify-center w-full gap-1.5 md:gap-3 h-full relative group-hover/bar:z-20">
                                            ...
                                            <div className="absolute bottom-full mb-6 opacity-0 group-hover/bar:opacity-100 transition-all duration-500 transform translate-y-4 group-hover/bar:translate-y-0 pointer-events-none z-30">
                                                <div className="bg-gray-900/95 backdrop-blur-md text-white p-5 rounded-3xl shadow-2xl border border-white/10 w-48 overflow-hidden relative">
                                                    ...
                                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3">{m} {new Date().getFullYear()}</p>
                                                    <div className="space-y-2">
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-[10px] text-gray-400 font-bold">Ingresos:</span>
                                                            <span className="text-xs font-black text-emerald-400">S/ {monthlyIncomeTotals[i].toLocaleString()}</span>
                                                        </div>
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-[10px] text-gray-400 font-bold">Egresos:</span>
                                                            <span className="text-xs font-black text-rose-400">S/ {monthlyExpenseTotals[i].toLocaleString()}</span>
                                                        </div>
                                                        <div className="pt-2 border-t border-white/10 mt-2 flex justify-between items-center">
                                                            <span className="text-[10px] text-gray-200 font-black">Neto:</span>
                                        ...
                                        <div className="h-14 flex items-center">
                                            <span className={`text-[11px] font-black tracking-tighter transition-colors ${monthlyBalances[i] >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                {m}
                                            </span>
                                        </div>
                                    </div>
                ...
                <div className="bg-white rounded-[2rem] shadow-[0_30px_80px_rgba(0,0,0,0.04)] border border-gray-100 overflow-hidden flex flex-col">
                    <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-b from-gray-50/50 to-white">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl md:text-2xl font-black text-gray-800 tracking-tight">Estructura Detallada por Meses</h2>
                                <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
                                    <ArrowRight className="w-3 h-3 animate-pulse" /> Desliza para navegar
                                </p>
                            </div>
                        </div>
                        <div className="flex gap-3">
                            <button onClick={expandAll} className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-100 transition-all border border-indigo-100">Expandir Todo</button>
                            <button onClick={collapseAll} className="px-4 py-2 text-gray-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all">Contraer</button>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-left border-collapse min-w-[1400px]">
                                <thead>
                                    <tr className="bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
                                        ...
                                        {months.map(m => (
                                            <th key={m} className="p-3 md:p-4 text-center text-[10px] md:text-xs font-black uppercase tracking-widest border-r border-gray-50 text-gray-400 w-20 md:w-28">
                                                {m}
                                            </th>
                                        ))}
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    ...
                                    <tr className="bg-gradient-to-r from-emerald-400 to-teal-500 group">
                                        <td className="sticky left-0 bg-gradient-to-r from-emerald-400 to-teal-500 z-30 p-3 md:p-4 pl-4 md:pl-8 border-r border-emerald-300 text-white font-black uppercase text-[10px] md:text-xs tracking-[0.1em] shadow-[10px_0_20px_-5px_rgba(16,185,129,0.3)] flex items-center gap-2 overflow-hidden">
                                            <TrendingUp className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0" /> <span className="truncate">INGRESOS</span>
                                        </td>
                                        {monthlyIncomeTotals.map((tot, i) => (
                                            <td key={`inc-tot-${i}`} className="p-3 md:p-4 text-center font-black text-white/90 text-[10px] md:text-sm border-r border-white/10 group-hover:bg-black/5 transition-colors">
                                                {formatSoles(tot)}
                                            </td>
                                        ))}
                                    ...
                                    {incomeRows.map((inc, i) => {
                                        ...
                                        return (
                                            <tr key={`inc-row-${i}`} className="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors group">
                                                ...
                                                {inc.values.map((v, j) => (
                                                    <td key={`inc-v-${j}`} className="p-2 md:p-3 text-center text-gray-600 font-bold text-[10px] md:text-[11px] border-r border-gray-100/50">
                                                        {formatSoles(v)}
                                                    </td>
                                                ))}
                                    ...
                                    <tr className="bg-gradient-to-r from-purple-400 to-pink-500 group">
                                        <td className="sticky left-0 bg-gradient-to-r from-purple-400 to-pink-500 z-30 p-3 md:p-4 pl-4 md:pl-8 border-r border-purple-300 text-white font-black uppercase text-[10px] md:text-xs tracking-[0.1em] shadow-[10px_0_20px_-5px_rgba(168,85,247,0.3)] flex items-center gap-2 overflow-hidden">
                                            <TrendingDown className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0" /> <span className="truncate">EGRESOS</span>
                                        </td>
                                        {monthlyExpenseTotals.map((tot, i) => (
                                            <td key={`exp-tot-${i}`} className="p-3 md:p-4 text-center font-black text-white/90 text-[10px] md:text-sm border-r border-white/10 group-hover:bg-black/5 transition-colors">
                                                {formatSoles(tot)}
                                            </td>
                                        ))}
                                    ...
                                    {expenseRows.map((cat, i) => {
                                        ...
                                        cat.subcategories.forEach(sub => {
                                            sub.values.forEach((v, monthIdx) => { catMonthlyTotals[monthIdx] += v; });
                                        });
                                                ...
                                                <tr className="border-b border-gray-100 hover:bg-purple-50/10 transition-colors cursor-pointer group" onClick={() => toggleCat(`exp-${i}`)}>
                                                    <td className="sticky left-0 bg-white/95 backdrop-blur-md group-hover:bg-purple-50/5 z-20 p-2 md:p-3 pl-4 md:pl-8 font-black text-gray-800 text-[10px] md:text-xs uppercase border-r border-gray-100 shadow-[8px_0_15px_-5px_rgba(0,0,0,0.05)] flex items-center justify-between transition-colors pr-2 md:pr-4">
                                                        <div className="flex items-center gap-2 overflow-hidden">
                                                            {isExpanded ? <ChevronDown className="w-4 h-4 text-purple-500 flex-shrink-0" /> : <ChevronRight className="w-4 h-4 text-gray-300 flex-shrink-0" />}
                                                            <span className="truncate" title={cat.category}>{cat.category}</span>
                                                        </div>
                                                    </td>
                                                    {catMonthlyTotals.map((tot, j) => (
                                                        <td key={`cat-tot-v2-${j}`} className={`p-2 md:p-3 text-center text-[10px] md:text-[11px] font-black border-r border-gray-100/50 ${tot > 0 ? 'text-gray-800' : 'text-gray-400'}`}>
                                                            {formatSoles(tot)}
                                                        </td>
                                                    ))}
                                                ...
                                                {isExpanded && cat.subcategories.map((sub, j) => {
                                                    ...
                                                    return (
                                                        <tr key={`exp-sub-v2-${j}`} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                                            ...
                                                            {sub.values.map((v, k) => (
                                                                <td key={`exp-v-v2-${k}`} className="p-1.5 md:p-2 text-center text-gray-500 font-medium text-[9px] md:text-[10px] border-r border-gray-50/50">
                                                                    {formatSoles(v)}
                                                                </td>
                                                            ))}
                                    ...
                                    <tr className={`border-t-4 border-white text-white shadow-[0_-20px_60px_rgba(0,0,0,0.2)] ${grandTotalBalance >= 0 ? 'bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600' : 'bg-gradient-to-r from-rose-600 via-pink-600 to-orange-600'}`}>
                                        <td className={`sticky left-0 z-40 p-4 md:p-6 pl-4 md:pl-8 font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-[10px] md:text-sm border-r border-white/20 shadow-[15px_0_35px_rgba(0,0,0,0.4)] ${grandTotalBalance >= 0 ? 'bg-emerald-600' : 'bg-rose-600'} flex items-center overflow-hidden`}>
                                            <span className="truncate">BALANCE NETO</span>
                                        </td>
                                        {monthlyBalances.map((bal, i) => (
                                            <td key={`bal-f2-${i}`} className={`p-4 md:p-6 text-center font-black text-[11px] md:text-sm border-r border-white/10 ${bal >= 0 ? 'bg-white/10' : 'bg-black/10 text-white/70'}`}>
                                                {formatSoles(bal)}
                                            </td>
                                        ))}
                                        <td className={`p-4 md:p-6 text-right font-black text-sm md:text-xl ${grandTotalBalance >= 0 ? 'bg-emerald-900/50' : 'bg-rose-900/50'}`}>
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-12 text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] pb-10 mt-6 opacity-50 hover:opacity-100 transition-opacity">
                    <div className="flex items-center gap-3"><Activity className="w-5 h-5 text-indigo-400" /> Sincronizado en tiempo real</div>
                    <div className="flex items-center gap-3"><Wallet className="w-5 h-5 text-indigo-400" /> Control Financiero Activo</div>
                    <div className="flex items-center gap-3"><Calendar className="w-5 h-5 text-indigo-400" /> Ciclo {new Date().getFullYear()}</div>
                </div>
