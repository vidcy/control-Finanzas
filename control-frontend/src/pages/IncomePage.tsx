...
export default function IncomePage() {
  ...
  const handlePdfExport = async () => {
    ...
    try {
      ...
      // Emerald/green banner for income
      doc.setFillColor(4, 120, 87);
      doc.rect(0, 0, 210, 32, "F");
      doc.setTextColor(255, 255, 255);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(15);
      doc.text("REPORTE DE INGRESOS", 14, 11);
      doc.setFontSize(8);
      ...
      // KPI summary
      const totalAmt = filtered.reduce((s: number, e: any) => s + (e.amount || 0), 0);
      const paidCount = filtered.filter((e: any) => e.status === "PAID").length;
      const pendingCount = filtered.filter((e: any) => e.status === "PENDING").length;
      ...
      const headers = ["Fecha", "Categoría", "Descripción / Fuente", "Método", "Estado", "Monto (S/)"];
      ...
      filtered.forEach((e: any, idx: number) => {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text("Control Finanzas ERP — Reporte de Ingresos", 14, 291);
        doc.text(`Página ${i} de ${totalPages}`, 185, 291);
      }

      doc.save(`Ingresos_${new Date().toISOString().slice(0, 10)}.pdf`);
      toast.dismiss(exportToast);
      toast.success("PDF de ingresos exportado");
    } catch (err) {
      console.error(err);
      toast.dismiss(exportToast);
