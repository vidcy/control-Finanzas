...
import { Users, Plus, Search, Edit2, ShieldAlert, CheckCircle, XCircle, Loader2, CheckCircle2, User as UserIcon, Mail, ShieldCheck, Activity } from "lucide-react";
...
export default function UserPage() {
    ...
    const [formData, setFormData] = useState({ name: "", lastName: "", email: "", role: "USER", password: "" });

    const handleOpenCreate = () => {
        setFormData({ name: "", lastName: "", email: "", role: "USER", password: "" });
        setIsCreateOpen(true);
    };

    const handleOpenEdit = (user: User) => {
        setSelectedUser(user);
        setFormData({ name: user.name, lastName: user.lastName, email: user.email, role: user.role, password: "" });
        setIsEditOpen(true);
    };
    ...
    return (
        <Appshell>
            <div className="flex flex-col gap-8 animate-fade-in-up pb-10">
                                    ...
                                    <tr key={user.id} className="hover:bg-purple-50/30 transition-all group">
                                        <td className="p-6 pl-8">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-black text-gray-800">{user.name} {user.lastName}</p>
                                        ...
                                        <td className="p-6">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider ${user.role === 'ADMIN' ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : 'bg-gray-100 text-gray-600 border border-gray-200'}`}>
                                ...
                                {!loading && filteredUsers.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="p-20 text-center">
                                            ...
                                            <p className="text-sm text-gray-400 font-bold uppercase tracking-widest">No se encontraron usuarios</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                {/* MODAL CREAR USUARIO */}
                <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Alta de Nuevo Usuario">
                    <form onSubmit={handleSaveCreate} className="space-y-8">
                        <div className="bg-purple-50/50 p-8 rounded-[2.5rem] border border-purple-100/50 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input required type="text" className="w-full px-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-gray-700 shadow-sm"
                                        value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="flex items-center gap-2 text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">
                                        <UserIcon className="w-3.5 h-3.5 text-purple-500" /> Apellidos
                                    </label>
                                    <input required type="text" className="w-full px-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-gray-700 shadow-sm"
                            ...
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">
                                    <Mail className="w-3.5 h-3.5 text-purple-500" /> Correo Electrónico
                                </label>
                                <input required type="email" className="w-full px-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-gray-700 shadow-sm"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="flex items-center gap-2 text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">
                                        <ShieldCheck className="w-3.5 h-3.5 text-purple-500" /> Rol Asignado
                                    </label>
                                    <select className="w-full px-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-black text-gray-700 shadow-sm appearance-none"
                                ...
                                <div className="space-y-2">
                                    <label className="flex items-center gap-2 text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">
                                        <Activity className="w-3.5 h-3.5 text-purple-500" /> Contraseña Temporal
                                    </label>
                                    <input required type="password" placeholder="••••••••" className="w-full px-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-gray-700 shadow-sm"
                        ...
                        <div className="flex justify-end gap-4 pt-2">
                            <button type="button" onClick={() => setIsCreateOpen(false)} className="px-8 py-4 text-gray-400 font-black uppercase text-xs tracking-widest hover:text-gray-600 transition-all">Cancelar</button>
                            <button type="submit" disabled={isSaving} className="px-10 py-4 bg-purple-600 text-white font-black rounded-2xl hover:bg-purple-700 shadow-xl shadow-purple-100 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2">
                ...
                <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Actualizar Datos de Usuario">
                    <form onSubmit={handleSaveEdit} className="space-y-8">
                        <div className="bg-indigo-50/50 p-6 rounded-[2rem] border border-indigo-100/50 flex items-start gap-4 mb-4">
                            ...
                            <div className="space-y-1">
                                <p className="text-sm font-black text-indigo-900 tracking-tight">Estás editando a: {selectedUser?.name} {selectedUser?.lastName}</p>
                                <p className="text-xs text-indigo-600/70 font-medium">La contraseña solo se actualizará si ingresas una nueva en el campo correspondiente.</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50/50 p-8 rounded-[2.5rem] border border-gray-100 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="password" placeholder="Sin cambios" className="w-full px-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-gray-700 shadow-sm"
                                        value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-4 pt-2">
                            <button type="button" onClick={() => setIsEditOpen(false)} className="px-8 py-4 text-gray-400 font-black uppercase text-xs tracking-widest hover:text-gray-600 transition-all">Cancelar</button>
                            <button type="submit" disabled={isSaving} className="px-10 py-4 bg-purple-600 text-white font-black rounded-2xl hover:bg-purple-700 shadow-xl shadow-purple-100 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2">
                                {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckCircle2 className="w-5 h-5" />}
